 ...
 */
function computedCoordDistance (startCoord: Coordinates, endCoord: Coordinates) {
 ...
 * @returns 去重数组 [{}]
 */
function uniqueBy<T extends {}> (arr: T[], key: string = 'name') {
  const res = new Map()
 ...
 * @returns string
 */
function formatTime (data: FormatTimeType) {
  const {
    date,
    format = 'yyyy-MM-dd',
    link = '-'
